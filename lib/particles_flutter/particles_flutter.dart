...
import '../particles_flutter/component/painter.dart';
...
class CircularParticle extends StatefulWidget {
  CircularParticle({
    ...
    this.hoverRadius = 80,
    this.connectDotsDistance = 50,
    this.connectDots = false,
    this.directionChangeTimeMili = 600,
  }) : super(key: key);
  ...
  final double connectDotsDistance;
  final bool connectDots; //not recommended
  final int directionChangeTimeMili;
  ...
  changeDirection() async {
    Future.doWhile(() async {
      await Future.delayed(
          Duration(milliseconds: widget.directionChangeTimeMili));
  ...
  connectLines() {
    ...
    for (int point1 = 0; point1 < offsets.length; point1++) {
      for (int point2 = 0; point2 < offsets.length; point2++) {
        ...
        if (distanceBetween < widget.connectDotsDistance) {
